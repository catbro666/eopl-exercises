#lang eopl
; Exercise 3.17 [**] Extend the language with a `let*` expression that works like Scheme’s `let*`
...
; no need to check the arity mismatch here
...
; test
;>  (run "let* x=1 y=+(x,1) in cons(x,y)")
;#(struct:pair-val #(struct:num-val 1) #(struct:num-val 2))
