...
; A `traceproc` works exactly like a `proc`, except that it prints a trace message on entry and on exit.

; Based on 3.22 version implementation of PROC.
; No need to add new datatype, just add a element in the compound to indicate if this is a traceproc
...
;4
;> (run "let f = traceproc (x y) (+ x y) in (f 3 1)")
;enter procedure
;exit procedure
;4