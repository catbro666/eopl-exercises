#lang eopl
; Exercise 3.31 [*] Extend the language above to allow the declaration of a recursive procedure
; of possibly many arguments
  ...
  (cases env-type env
    ...
    (extend-env-rec (p-name b-vars p-body saved-env)
                    (if (eqv? p-name search-var)
                        (proc-val (compound b-vars p-body env #f))
                        (apply-env saved-env search-var)))))
...
; test
;> (run "letrec fact(x p) = if (zero? x) then p else (fact (- x 1) (* x p)) in (fact 4 1)")
;24