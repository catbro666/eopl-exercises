#lang eopl
; CALL-BY-RERERENCE (based on 4.39 version)
...
; ExpVal and DenVal have no change under call-by-reference.
...
; under call-by-reference, a new location is created for every evaluation of an operand other than a variable.

; Note because our language support both mutable and immutable variables, so only the mutable variables
; are call-by-reference.

; We add procedure `value-of-rands` to address the parameter-passing
; and call it when evaluating the `nameless-call-exp`.
  ...
  (let* ((vecs (map (lambda (n) (make-vector 1)) exps))
    ...
    (for-each (lambda (vec exp)
                (cases expression exp
                  (nameless-proc-exp (body mode) (vector-set! vec 0 (value-of exp new-env)))
...
; also need to extract the vector element
  ...
  (cases expression exp
    ...
    (call-exp (rator rands)
              (let* ((a-proc (expval->proc (value-of rator env)))
                     (args (value-of-rands a-proc rands env)))
...
; ListOf(Exp) × Env → ListOf(Ref) / ListOf(Expval)
; if the operand is a mutable variable (alrady a reference) we don't create new location.
(define (value-of-rands rator rands env)
  (cases proc rator
    (compound (body saved-env trace mode)
              (if (= mode 0)
                  (map newref (value-of-explist rands env))  ; call-by-value
                  (map (lambda (rand)           
                         (cases expression rand
                           (nameless-var-exp (addr)
                                             (let ((denval (apply-nameless-env env addr)))
                                               (if (reference? denval)
                                                   denval  ; call-by-reference
                                                   (newref denval)))) ; call-by-value
                           (else (newref (value-of rand env)))))
                       rands)))
    (primitive (name op) (value-of-explist rands env)))) ; we don't newref for primitives
