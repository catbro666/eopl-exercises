#lang eopl
; Continuation-Passing Interpreter (based on 5.11 version)

;Exercise 5.16 [**]  Extend the continuation-passing interpreter to the language of exercise 4.22
...
;(define (run string)
;  (expval->sexp (result-of-program (scan&parse string))))

(define (run string)
  (result-of-program
   (scan&parse string)))

;result-of-program : Program → Effect
(define (result-of-program pgm)
  (initialize-store!)
  (cases program pgm
    (a-program (state1)
               (result-of/k state1 (init-env) (end-cont))
               (let ((x 0)) (set! x 1)) ; just to return void
               )))

(define (result-of/k state env cont)
  (cases statement state
    (assign-state (addr exp1)
                           (let ((ref (apply-env env addr)))
                             (value-of/k exp1 env
                                         (lambda (val)
                                           (setref! ref val)
                                           (apply-command-cont cont)))))
    (print-state (exp1) (value-of/k exp1 env
                                    (lambda (val)
                                      (display (expval->sexp val))
                                      (newline)
                                      (apply-command-cont cont))))
    (multi-state (states) (let loop ((states states))
                            (if (null? states)
                                (apply-command-cont cont)
                                (result-of/k (car states) env
                                             (lambda ()
                                               (loop (cdr states)))))))
    (if-state (exp1 state1 state2)
              (value-of/k exp1 env
                          (lambda (val)
                            (if (expval->bool val)
                                (result-of/k state1 env cont)
                                (result-of/k state2 env cont)))))
    (while-state (exp1 state1) (let loop ()
                                 (value-of/k exp1 env
                                             (lambda (val)
                                               (if (expval->bool val)
                                                   (result-of/k state1 env loop)
                                                   (apply-command-cont cont))))))
    (varblock-state (vars exps state1)
                             (let loop ((vars vars)
                                        (exps exps)
...
(define repl
  (sllgen:make-rep-loop "--> " result-of-program
                        (sllgen:make-stream-parser scanner-spec grammar)))

; test
(run "print 1")
;1
(run "var a=1,b=2; print (cons a b)")
;(1 . 2)
(run "var a=1,b=2; {a=3; b=4;print (cons a b)}")
;(3 . 4)
(run "var a=1; if (zero? a) print 2 print 3")
;3
(run "var a=0; if (zero? a) print 2 print 3")
;2
(run "var a=2; while (not (zero? a)) {print a; a=(- a 1)}")
;2 1