#lang eopl
; Exercise 3.22 [***]  Modify the concrete syntax so that the user of this language need not know
; which operations are built-in and which are defined procedures.

; Note to avoid confusion we also removed `,` in the `proc` syntax
; And we add the Primitives as a subtype of `proc` to distinguish it from the scheme procedure.
; Now we can remove the `op-exp` and merge it into the `call-exp`. The builtin operations are bound to
; their names in advance in the global env.
; Besides, we also need to expand the scope of identifier so that it includes our primitives
...
(define value-of
  (lambda (exp env)
    (cases expression exp
      ...
      (proc-exp (vars body) (proc-val (compound vars body env)))
      (call-exp (rator rands)
