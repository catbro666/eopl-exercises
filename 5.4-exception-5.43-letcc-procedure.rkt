...
;Exercise 5.43 [**]  Modify `letcc` as defined in the preceding exercise so that the
;captured continuation becomes a new kind of procedure, so instead of writing
;`throw exp1 to exp2`, one would write `(exp2 exp1)`.
...
(define-datatype proc proc?
  ...
  (ccproc (saved-cont continuation?)))
  ...
  (cases expval val
    ...
    (proc-val (a-proc) (cases proc a-proc
                         ...
                         (primitive (name op) (string->symbol (string-append "#<primitive:"
                                                                             (symbol->string name) ">")))
                         (ccproc (saved-cont) (string->symbol "#<ccproc>"))))
...
(define value-of/k
  (lambda (exp env cont)
    (cases expression exp
      ...
      (letcc-exp (var exp1)
                 (value-of/k exp1 (extend-env var (proc-val (ccproc cont)) env) cont))
      (call-exp (rator rands)
...
(define (apply-procedure/k proc1 args cont)
  (lambda ()
    (cases proc proc1
      ...
      (ccproc (saved-cont) (apply-cont saved-cont (car args))) ; must have and only have 1 arg
      )))
