#lang eopl
; Lazy-Evaluation (call-by-name, call-by-need) (based on 4.35 version)
;Exercise 4.39 [*] In the absence of effects, call-by-name and call-by-need always give the same answer.
;Construct an example in which call-by-name and call-by-need give different answers.
...
;                   Skip (just change the `mode` and `mutable` of nameless-let-exp)

; introduce a new data type `thunk`, and let our denoted values can be Ref(thunk)
; in order to support call-by-name, call-by-value, call-by-reference simultaneously
; we add special form `procname` for call-by-name, `procneed` for call-by-need

;ExpVal  = Int+Bool+Null+Pair+Proc+Void+Subr+MutPair+ArrVal+Ref(ExpVal)
;DenVal  = Ref(ExpVal) + ExpVal + [Vector(DenVal)] + Ref(Thunk)
...
;               proc-exp (vars body)
;Expression ::= procref ({Identifier}*) Expression
;               procref-exp (vars body)
;Expression ::= procname ({Identifier}*) Expression
;               procname-exp (vars body)
;Expression ::= procneed ({Identifier}*) Expression
;               procneed-exp (vars body)
...
;               nameless-let-exp (exps body mode mutable)
;Expression ::= %let* {Expression}* in Expression
...
;Expression ::= %lexproc Expression Integer
;               nameless-proc-exp (body mode)
...
(define grammar
  '((program (statement) a-program)
    ...
    (expression ("procref" "(" (arbno identifier) ")" expression) procref-exp)
    (expression ("procname" "(" (arbno identifier) ")" expression) procname-exp)
    (expression ("procneed" "(" (arbno identifier) ")" expression) procneed-exp)
    ...
    (expression ("%ref" number) nameless-ref-exp)
    (expression ("%let" (arbno expression) "in" expression number number) nameless-let-exp)
    (expression ("%let*" (arbno expression) "in" expression number number) nameless-let*-exp)
    (expression ("%letrec" (arbno expression) "in" expression number number) nameless-letrec-exp)
    (expression ("%unpack" expression "in" expression) nameless-unpack-exp)
    (expression ("%lexproc" expression number) nameless-proc-exp)
...
(define-datatype proc proc?
  ; mode: 0-call-by-value, 1-call-by-reference
  (compound (body expression?) (saved-nameless-env nameless-environment?) (trace boolean?) (mode integer?))
...
(define-datatype thunk thunk?
  (a-thunk (exp1 expression?) (env nameless-environment?) (memo boolean?)))

(define (mthunk? th)
  (cases thunk th
    (a-thunk (exp1 env memo) memo)))
...
(define (is-proc/subr? exp)
  (cases expression exp
    (proc-exp (vars body) #t)
    (procref-exp (vars body) #t)
    (procname-exp (vars body) #t)
    (procneed-exp (vars body) #t)
    (traceproc-exp (vars body) #t)
    (subr-exp (vars body) #t)
    (else #f)))
...
(define (extend-senv* vars exps senv)
  (let loop ((vars vars)
             ...
             (nexps '()))
    (if (null? vars)
        (cons senv (reverse nexps))
        (loop (cdr vars) (cdr exps) (extend-senv (car vars) senv)
              (cons (translation-of-exp (car exps) senv) nexps)))))

; similar to extend-senv* , except if the current exp being translated is a proc,
; then extend the following procs before translating
; Listof(Var) x Listof(Exp) x Senv -> Pairof(Senv . Listof(NamelessExp))
(define (extend-senv-rec vars exps senv)
  (let loop ((vars vars)
             (exps exps)
             (senv senv)
             (nexps '()))
    (if (null? vars)
        ...
        (let ((var (car vars))
              ...
              (rest-vars (cdr vars))
              (rest-exps (cdr exps)))
          (let* ((next-senv (extend-senv var senv))
                 (trans-senv (if (is-proc/subr? exp)
                                 (extend-senv+ (filter (lambda (var exp)
                                                         (is-proc/subr? exp))
                                                       rest-vars rest-exps)
                                               next-senv)
                                 senv)))
            (loop rest-vars rest-exps next-senv
                  (cons (translation-of-exp exp trans-senv) nexps)))))))
...
(define (translation-of-state state senv)
  (cases statement state
    ...
    ; similar to letrec-exp, the scope of a variable includes the body,
    ; all the initializers in the front, proc initializers in the back
    (varblock-state (vars exps state1)
                    (let ((res (extend-senv-rec vars exps senv)))
                      (nameless-varblock-state (cdr res) (translation-of-state state1 (car res)))))
...
(define (translation-of-exp exp senv)
  (cases expression exp
    ...
    (ref-exp (var) (nameless-ref-exp (apply-senv senv var)))
    (if-exp (exp1 exp2 exp3)
            (if-exp
             (translation-of-exp exp1 senv)
    ...
    (let*-exp (vars exps body)
              (let ((res (extend-senv* vars exps senv)))
                (nameless-let*-exp (cdr res) (translation-of-exp body (car res)) 0 0))) ;cbv/immutable
    (letrec-exp (vars exps letrec-body)
                (let ((res (extend-senv-rec vars exps senv)))
                  (nameless-letrec-exp (cdr res) (translation-of-exp letrec-body (car res)) 0 0))) ;cbv/immutable
    (null-exp () (null-exp))
    ...
    (unpack-exp (vars exp1 body)
                (nameless-unpack-exp
                 (translation-of-exp exp1 senv)
                 (translation-of-exp body (extend-senv+ vars senv))))
    (proc-exp (vars body) (nameless-proc-exp (translation-of-exp body (extend-senv+ vars senv)) 0))
    (procref-exp (vars body) (nameless-proc-exp (translation-of-exp body (extend-senv+ vars senv)) 1))
    (procname-exp (vars body) (nameless-proc-exp (translation-of-exp body (extend-senv+ vars senv)) 2))
    (procneed-exp (vars body) (nameless-proc-exp (translation-of-exp body (extend-senv+ vars senv)) 3))
...
(define (apply-nameless-env env addr)
  (may-vector-ref (list-ref env addr)))

;set-nameless-env : Nameless-env × Lexaddr × DenVal → Unspecified
(define (set-nameless-env env addr val)
  (let loop ((env env)
             (addr addr))
    (if (= addr 0)
        (cons val (cdr env))
        (cons (car env) (loop (cdr env) (- addr 1))))))

(define (extend-nameless-env+ vals env)
  (fold/l (lambda (env val) (extend-nameless-env val env))
          env vals))

(define (extend-nameless-env++ exps env mode mutable)
  (extend-nameless-env+ (map (lambda (exp) (value-of-rand-mut exp env mode mutable)) exps) env))

; each exp is evaluated in the env just extended in the previous step
(define (extend-nameless-env* exps env mode mutable)
  (fold/l (lambda (env exp)
            (extend-nameless-env (value-of-rand-mut exp env mode mutable) env))
          env exps))

(define (is-nameless-proc/subr? exp)
  (cases expression exp
    (nameless-proc-exp (body mode) #t)
    (nameless-traceproc-exp (body) #t)
    (nameless-subr-exp (body) #t)
    (else #f)))

; support procs and other expressions simultaneously
(define (extend-nameless-env-rec exps saved-env mode mutable)
  (let ((vecs (map (lambda (exp) (make-vector 1)) exps)))
    (let loop ((vecs vecs)
               (exps exps)
               (env saved-env))
      (if (null? vecs)
          ...
          (let ((vec (car vecs))
                ...
                (rest-vecs (cdr vecs))
                (rest-exps (cdr exps)))
            (let* ((next-env (extend-nameless-env vec env))
                   (eval-env (if (is-nameless-proc/subr? exp)
                                 (extend-nameless-env+ (filter (lambda (vec exp)
                                                                 (is-nameless-proc/subr? exp))
                                                               rest-vecs rest-exps)
                                                       next-env)
                                 env))
                   (val (value-of-rand-mut exp eval-env mode mutable)))
              (vector-set! vec 0 val)
              (loop rest-vecs rest-exps next-env)))))))
  ...
  (cases statement state
    ...
    (dowhile-state (state1 exp1) (let loop ()
                                   ...
                                   (if (expval->bool (value-of exp1 env))
                                       ...
                                       'ok)))
    (nameless-varblock-state (exps state1)
                             (result-of state1 (extend-nameless-env-rec exps env 0 1))) ; cbv/mutable
...
(define (value-of exp env)
  (cases expression exp
    ...
    (nameless-var-exp (addr) (may-deref (apply-nameless-env env addr)))
    (nameless-ref-exp (addr) (apply-nameless-env env addr))
    (if-exp (exp1 exp2 exp3)
    ...
    (nameless-let-exp (exps body mode mutable)
                      (value-of body
                                (extend-nameless-env++ exps env mode mutable)))
    (nameless-let*-exp (exps body mode mutable)
                       (value-of body (extend-nameless-env* exps env mode mutable)))
    (nameless-letrec-exp (exps letrec-body mode mutable)
                         (value-of letrec-body (extend-nameless-env-rec exps env mode mutable)))
    (null-exp () (null-val))
    (cond-exp (preds actions)
    ...
    (nameless-proc-exp (body mode) (proc-val (compound body env #f mode)))
    (nameless-traceproc-exp (body) (proc-val (compound body env #t 0)))
    ...
    (call-exp (rator rands)
              (let* ((a-proc (expval->proc (value-of rator env)))
                     (args (cases proc a-proc
                             (compound (body saved-env trace mode) (value-of-rands rands env mode))
                             (primitive (name op) (value-of-explist rands env))))) ; no need to call-by-reference for primitives
...
; ListOf(Exp) × Env × Mode → ListOf(Ref/ExpVal)
; if the operand is a mutable variable (alrady a reference) we don't create new location.
(define (value-of-rands rands env mode)
  (map (lambda (rand) (value-of-rand-mut rand env mode 1)) rands))

; if the varible is mutable, we should make sure the return value is a ref
(define (value-of-rand-mut rand env mode mutable)
  (make-mutable (value-of-rand rand env mode) mutable))

; Exp × Env × Mode → Ref/ExpVal
; mode: 0 - call-by-value, 1 - call-by-reference
; if the denoted value is a reference, return the location directly
(define (value-of-rand rand env mode)
  (case mode
    ((0) (value-of rand env))   ; call-by-value
    ((1) (cases expression rand ; call-by-reference
           (nameless-var-exp (addr) (apply-nameless-env env addr))
           (else (value-of rand env))))
    ((2 3) (cases expression rand ; call-by-name
           ; no need to freeze these exps because they have no effect and their values won't change
           (const-exp (num) (num-val num))
           (nameless-var-exp (addr) (apply-nameless-env env addr))
           (nameless-ref-exp (addr) (apply-nameless-env env addr))
           (null-exp () (null-val))
           ...
           (nameless-traceproc-exp (body) (proc-val (compound body env #t 0)))
           (nameless-subr-exp (body) (subr-val (comp-subr body env)))
           (else (a-thunk rand env (= mode 3)))))))

(define (value-of-mut exp env mutable)
  (value-of-rand-mut exp env 0 mutable))

; make a expval mutable by adding a reference
(define (make-mutable val mutable)
  (if (and (= 1 mutable) (not (reference? val)))
      (newref val)
      val))

(define (value-of-thunk ref th)
  (cases thunk th
    (a-thunk (exp1 env memo)
             (let ((res (value-of exp1 env)))
               (if memo
                   (setref! ref res)
                   #t)
               res))))
...
(define (apply-subroutine subr1 args)
  (cases subr subr1
    (comp-subr (body env)
               ;(check-arity vars args)
               (result-of body (extend-nameless-env+ args env)))))
...
; if is a vector, extract the element
(define (may-vector-ref denval)
  (if (vector? denval)
      (vector-ref denval 0)
      denval))

; if is a ref, deref it
; impossible for a ref in thunk (we only create thunk for non-var exps)
(define (may-deref denval)
  (cond
    ((reference? denval) (let ((val (deref denval)))
                           (if (thunk? val)
                               (value-of-thunk denval val)
                               val)))
    (else denval)))
...
(run "var odd=proc (x) if (zero? x) then false else (even (- x 1)), i=(+ 1 i),
          even=proc (x) if (zero? x) then true else (odd (- x 1)), x =(odd i), y = (even i);
          {print (cons x y)}")
;(#f . #t)
(run "print letrec odd=proc (x) if (zero? x) then false else (even (- x 1)) i=(+ 1 i)
...
;0
;1
(run "print let f = proc (x) set x = 44
      in let g = proc (y) (f y)
         in letm z = 55
            in begin (g z) z end")
;55
(run "print let f = proc (x) set x = 44
      in let g = procref (y) (f y)
         in letm z = 55
            in begin (g z) z end")
;55
(run "print let f = procref (x) set x = 44
      in let g = proc (y) (f y)
         in letm z = 55
            in begin (g z) z end")
;55
...
(run "print letm a=3 b=4
            ...
            in begin (swap ref a ref b) (cons a b) end")
;(4 . 3)

(run "print letm x = 0 in
              let f = proc () begin set x = (+ x 1) x end in
                let g = proc(y) begin set x = 10 proc() y end in
                   let h = (g (f)) in
                     (cons (h) (h))")
; (1 . 1) call-by-value
(run "print letm x = 0 in
              let f = proc () begin set x = (+ x 1) x end in
                let g = procname(y) begin set x = 10 proc() y end in
                   let h = (g (f)) in
                     (cons (h) (h))")
; (11 . 12) call-by-name
(run "print letm x = 0 in
              let f = proc () begin set x = (+ x 1) x end in
                let g = procneed(y) begin set x = 10 proc() y end in
                   let h = (g (f)) in
                     (cons (h) (h))")
; (11 . 11) call-by-need