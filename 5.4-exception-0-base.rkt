    ...
    (cases expression exp
      ...
      (let-exp (vars exps body)
               (value-of-explist/k exps '() env
                                   (explist-final-cont vars body env cont)))
      (let*-exp (vars exps body)
                (value-of-explist*/k vars exps env
                                     (explist*-final-cont body cont)))
      (letrec-exp (p-names b-varss p-bodys letrec-body)
                  (value-of-explist-rec/k p-names b-varss p-bodys env
                                          (explist-rec-final-cont letrec-body cont)))
...
(define (value-of-explist/k exps vals env cont)
  (if (null? exps)
      ...
      (value-of/k (car exps) env (explist-cont (cdr exps) vals env cont))))

(define (value-of-explist*/k vars exps env cont)
  (if (null? exps)
      (apply-cont cont env)
      (value-of/k (car exps) env (explist*-cont vars (cdr exps) env cont))))
...
(define (value-of-cond/k preds actions env cont)
  (if (null? preds)
      ...
      (value-of/k (car preds) env
                  (cond-exp-cont (cdr preds) actions env cont))))
