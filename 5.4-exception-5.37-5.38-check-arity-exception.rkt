...
(define (check-arity vars args savec-try-cont)
  (let ((l1 (length vars))
        (l2 (length args)))
    (if (not (= l1 l2))
        (apply-handler -99 savec-try-cont)
  ...
  (lambda ()
    (cases proc proc1
      (compound (vars body env trace)
                ...
                (check-arity vars args (get-saved-try-cont cont))
...
;                        else (/ x y)
;      in (div 6)")
;;apply-handler: uncaught exception: -99