#lang eopl
; Implicit References (Based on 4.22 version)

;Exercise 4.23 [*] Add to the language of exercise 4.22 read statements of the form `read var`.
; This statement reads a nonnegative integer from the input and stores it in the given variable.
;Exercise 4.24 [*] Add `do-while` statements to the language of exercise 4.22.
;Exercise 4.25 [*] Extend the block statement of the language of exercise 4.22 to allow variables
;to be initialized. In your solution, does the scope of a variable include the initializer
;for variables declared later in the same block statement? Yes!
...
(define (translation-of-state state senv)
  (cases statement state
    ...
    ; similar to let*-exp, the scope of a variable includes the initializer for the later variables
    (varblock-state (vars exps state1) (let ((res (extend-senv* vars exps senv)))
  ...
  (cases statement state
    ...
    (dowhile-state (state1 exp1) (let loop ()
                                   ...
                                   (if (expval->bool (value-of exp1 env))
                                       ...
                                       'ok)))
    (nameless-varblock-state (exps state1)
                             (result-of state1 (extend-nameless-env* exps env #t))) ; mutable
