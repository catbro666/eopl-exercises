#lang eopl
; Exercise 3.9 [**] Add list processing operations to the language, including `cons`, `car`, `cdr`, `null?` and `emptylist`.
; A list should be able to contain any expressed value, including another list.
; Add two types of expressed value: pair and null
...
;Operation  ::= +|-|*|/|equal?|greater?|less?|minus|cons|car|cdr|null?
...
; test
;> (run "cons(1, zero?(0))")
;#(struct:pair-val #(struct:num-val 1) #(struct:bool-val #t))
;> (run "car(cons(1, zero?(0)))")
;#(struct:num-val 1)
;> (run "cdr(cons(1, zero?(0)))")
;#(struct:bool-val #t)
;> (run "car(cons(1, cons(2, emptylist)))")
;#(struct:num-val 1)
;> (run "cons(1, cons(2, emptylist))")
;#(struct:pair-val #(struct:num-val 1) #(struct:pair-val #(struct:num-val 2) #(struct:null-val)))
;> (run "null?(cdr(cons(1, cons(2, emptylist))))")
;#(struct:bool-val #f)
;> (run "null?(cdr(cdr(cons(1, cons(2, emptylist)))))")
;#(struct:bool-val #t)
;(run "let x = 4
;      in cons(x, cons(cons(-(x,1), emptylist), emptylist))")
;#(struct:pair-val
;  #(struct:num-val 4)
;  #(struct:pair-val #(struct:pair-val #(struct:num-val 3) #(struct:null-val)) #(struct:null-val)))
