#lang eopl
; Exception (based on Exercise 5.42)
...
; is called `call-with-current-continuation`, takes a one-argument procedure, `p`,
;and passes to `p` a procedure that when invoked with one argument, passes that argument
;to the current continuation, `cont`. We could define `call-with-current-continuation`
;in terms of `letcc` and `throw` as follows:
;let call-with-current-continuation
;= proc (p)
;letcc cont
;in (p proc (v) throw v to cont)
;in ...
;Add `call-with-current-continuation` to the language. Then write a translator that
;takes the language with `letcc` and` throw` and translates it into the language
;without `letcc` and `throw`, but with `call-with-current-continuation`.

; translator:
;replace `letcc var in body` with `cwcc proc (var) body` and
;replace `throw exp1 to exp2` with `(exp2 exp1)` 
