#lang eopl
; Thread (based on Exercise 5.52)
; Exercise 5.53 [***] Modify the thread package to include thread identifiers. Each new
;thread is associated with a fresh thread identifier. When the child thread is spawned,
...
;spawn expression. Instrument the interpreter to trace the creation of thread identifiers.
;Check to see that the ready queue contains at most one thread for each thread identifier.
;How can a child thread know its parent’s identifier? What should be done
;about the thread identifier of the original program?

; add a new field `id` in the `thread` datatype
; pass the parent id to child thread when it is being created.
; add debug log in the `run-next-thread`
...
(define initialize-scheduler!
  (lambda (ticks)
    ...
    (set! the-current-thread 'uninitialized)))
