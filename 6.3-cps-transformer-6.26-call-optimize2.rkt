...
;occur once in the continuation. Modify make-send-to-cont so that instead of generating
;let var1 = simp1
;in T
...
; add `substitue-freevar` to do this substitution
;(transform "(zero? let x=1 in x)")

;(transform "(zero? let x=1 fn=proc(var1 var2) 1 in x)")
;#(struct:cps-a-program
;  #(struct:cps-let-exp
;    (x fn)
;    (#(struct:cps-const-exp 1) #(struct:cps-proc-exp (var1 var2 k%00) #(struct:cps-call-exp #(struct:cps-var-exp k%00) (#(struct:cps-const-exp 1)))))
;    #(struct:cps-let-exp
;      (var2)
;      (#(struct:cps-var-exp x))
;      #(struct:cps-call-exp #(struct:cps-var-exp zero?) (#(struct:cps-var-exp var2) #(struct:cps-proc-exp (var1) #(struct:simple-exp->exp #(struct:cps-var-exp var1))))))))
...
;    (x fn)
;   (#(struct:cps-const-exp 1) #(struct:cps-proc-exp (var1 var2 k%00) #(struct:cps-call-exp #(struct:cps-var-exp k%00) (#(struct:cps-const-exp 1)))))
;    #(struct:cps-call-exp #(struct:cps-var-exp zero?) (#(struct:cps-var-exp x) #(struct:cps-proc-exp (var1) #(struct:simple-exp->exp #(struct:cps-var-exp var1)))))))
