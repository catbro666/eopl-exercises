#lang eopl
; Implicit References (Based on 4.10 version)
;In this design, every variable denotes a reference. Denoted values are references to locations that contain
; expressed values. References are no longer expressed values. They exist only as the bindings of variables.
...
;Exercise 4.19 [**] Modify the implementation of multiprocedure letrec so that each closure is built only once,
;                   and only one location is allocated for it. This is like exercise 3.35.
...
(define value-of
  (lambda (exp env)
    (cases expression exp
      ...
      (nameless-var-exp (addr) (deref (apply-nameless-env env addr))) ;****
      (if-exp (exp1 exp2 exp3)
      ...
      ;****
      (nameless-assign-exp (addr exp1)
                           (begin (setref! (apply-nameless-env env addr) (value-of exp1 env))
                                  (void-val)))
      (call-exp (rator rands)
