...
;Exercise 4.35 [**] We can get some of the benefits of call-by-reference without leaving the call-by-value
;framework. Add `ref` special form. This differs from the language EXPLICIT-REFS,
;since references are only of variables

; Now the ExpVal can be Ref as well.

; we still implement this in the implicit reference language. So the variable describled should be mutable.

;ExpVal  = Int+Bool+Null+Pair+Proc+Void+Subr+MutPair+ArrVal+Ref(ExpVal)
;DenVal  = Ref(ExpVal) + ExpVal + Vector(DenVal)
...
(define (value-of-rand rand env mode)
  (if (= mode 0)
      (value-of rand env) ; call-by-value
      (cases expression rand
        (nameless-var-exp (addr) (apply-nameless-env env addr)) ; call-by-reference
