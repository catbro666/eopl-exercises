...
;Exercise 5.41 [***] We have shown how to implement exceptions using a datastructure representation of continuations.
;We can’t immediately apply the recipe of section 2.2.3 to get a procedural representation,
;because we now have two observers: `apply-handler` and `apply-cont`. Implement the continuations
;of this section as a pair of procedures: a one-argument procedure representing the action of
;the continuation under `apply-cont`, and a zero-argument procedure representing its action under `apply-handler`.
...
(define (apply-handler val cont saved-try-cont)
  ((saved-try-cont 'apply-handler) val cont))

(define (get-saved-try-cont cont)
  ((cont 'get-saved-try-cont)))
...
(define (end-cont)
  (letrec ((self
            (lambda (tag)
              (case tag
                ((apply-cont)
                 (lambda (val) (eopl:printf "End of computation.~%") val))
                ((apply-handler)
                 (lambda (val cont) (report-uncaught-exception)))
                ((get-saved-try-cont)
                 (lambda () self))))))
    self))

(define (if-test-cont exp2 exp3 saved-env saved-cont saved-try-cont)
  (lambda (tag)
    (case tag
      ((apply-cont)
       (lambda (val)
      ...
      ((apply-handler)
       (lambda (val cont) (report-invalid-try-continuation)))
      ((get-saved-try-cont)
       (lambda () saved-try-cont)))))

(define (explist-final-cont vars body saved-env saved-cont saved-try-cont)
  (lambda (tag)
    (case tag
      ((apply-cont)
       (lambda (val)
...
(define (explist-cont exps vals saved-env saved-cont saved-try-cont)
  (lambda (tag)
    (case tag
      ((apply-cont)
       (lambda (val)
...
(define (explist*-final-cont body saved-cont saved-try-cont)
  (lambda (tag)
    (case tag
      ((apply-cont)
       (lambda (val)
...
(define (explist*-cont vars exps saved-env saved-cont saved-try-cont)
  (lambda (tag)
    (case tag
      ((apply-cont)
       (lambda (val)
...
(define (explist-rec-final-cont body saved-cont saved-try-cont)
  (lambda (tag)
    (case tag
      ((apply-cont)
       (lambda (val)
...
(define (cond-exp-cont preds actions saved-env saved-cont saved-try-cont)
  (lambda (tag)
    (case tag
      ((apply-cont)
       (lambda (val)
...
(define (unpack-exp-cont vars body saved-env saved-cont saved-try-cont)
  (lambda (tag)
    (case tag
      ((apply-cont)
       (lambda (val)
...
(define (begin-exp-cont exps saved-env saved-cont saved-try-cont)
  (lambda (tag)
    (case tag
      ((apply-cont)
       (lambda (val)
...
(define (rator-cont rands saved-env saved-cont saved-try-cont)
  (lambda (tag)
    (case tag
      ((apply-cont)
       (lambda (val)
...
(define (rands-cont proc1 saved-cont saved-try-cont)
  (lambda (tag)
    (case tag
      ((apply-cont)
       (lambda (val)
...
(define (trace-cont trace saved-cont saved-try-cont)
  (lambda (tag)
    (case tag
      ((apply-cont)
       (lambda (val)
...
(define (try-cont var1 var2 handler-exp saved-env saved-cont)
  (letrec ((self
            (lambda (tag)
              (case tag
                ((apply-cont)
...
(define (raise-cont saved-cont saved-try-cont)
  (lambda (tag)
    (case tag
      ((apply-cont)
       (lambda (val)
...
(define (resume-cont saved-cont saved-try-cont)
  (lambda (tag)
    (case tag
      ((apply-cont)
       (lambda (val)
