#lang eopl
; Lexical Addressing
; Exercise 3.38 [*] Extend to handle `cond` from exercise 3.12
; Exercise 3.39 [*] Extend to handle `unpack` from exercise 3.18
; Exercise 3.40 [**] Extend to handle `letrec`. We use different way from the suggested.
; Exercise 3.41 [**] Modify to handle `let`, procedures, and calls with multiple arguments, as in exercise 3.21
;                    We don't use the depth/position representation.

; translation-of takes a program and removes all the variables from the declarations,
; and replaces every variable reference by its lexical depth.
; new productions: %lexref, %let, %let*, %letrec, %unpack, %lexproc, %lextraceproc
