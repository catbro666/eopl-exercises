...
; Currently we only have a global the-time-remaining. To achieve Ex. 5.46, we need to bind
; the-time-remaining to every thread. We introduce a new datatype `thread` to save this.
; Note we also need to create the main thread in `value-of-program`

;Exercise 5.47 [*] What happens if we are left with two subthreads, each waiting for
;a mutex held by the other subthread? Deadlock.
;Exercise 5.48 [*] We have used a procedural representation of threads. Replace this
;by a data-structure representation. Already a data-structure representation.
    ...
    (cases thread the-current-thread
      (a-thread (thunk time-remaining)
                (set! the-current-thread
                      (a-thread thunk (- time-remaining 1)))))))
