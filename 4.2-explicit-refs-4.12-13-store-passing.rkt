#lang eopl
; Exercise 4.12[***] Our understanding of the store, as expressed in this interpreter, depends on the meaning
; of effects in Scheme. In particular, it depends on us knowing when these effects take place in a Scheme program.
; We can avoid this dependency by writing an interpreter that more closely mimics the specification.
...
; we implement the newref/deref/setref! as special primitives with an additional parameter `store`

;ExpVal = Int+Bool+Null+Pair+Proc+Ref(ExpVal)+Void
;DenVal = ExpVal
...
(define-datatype proc proc?
  ...
  (store-prim (name identifier?) (op procedure?)))
  ...
  (cases expval val
    ...
    (proc-val (a-proc) (cases proc a-proc
                         ...
                         (primitive (name op) (string->symbol (string-append "#<primitive:"
                                                                             (symbol->string name) ">")))
                         (store-prim (name op) (string->symbol (string-append "#<primitive:"
                                                                              (symbol->string name) ">")))))
...
(define (init-senv)
  (fold/l (lambda (senv p)
          ...
          (fold/l (lambda (senv p)
                    (extend-senv (car p) senv)) (empty-senv) (append primitives store-primitives))
          global-bindings))
...
; -> PairOf(env, store)
(define (extend-nameless-env* exps env store)
  (if (null? exps)
      (cons env store)
      (cases answer (value-of (car exps) env store)
        (an-answer (val new-store)
                   (extend-nameless-env* (cdr exps) (extend-nameless-env val env) new-store)))))
...
(define (init-nameless-env)
  (fold/l (lambda (env p) (extend-nameless-env (cdr p) env))
          (fold/l (lambda (env p)
                    (let ((name (car p))
                          (op (cdr p)))
                      (extend-nameless-env (proc-val (store-prim name op)) env)))
                  (fold/l (lambda (env p)
                            (let ((name (car p))
                                  (op (cdr p)))
...
;newref : ExpVal → Answer(Ref, NewStore)
(define (prim-newref store val)
  (let ((next-ref (length store)))
    (an-answer (ref-val next-ref) (append store (list val)))))

;deref : Ref → Answer(ExpVal,Store)
(define (prim-deref store val)
  (let ((ref (expval->ref val)))
    (an-answer (list-ref store ref) store)))

;setref! : Ref × ExpVal → Answer(Unspecified, NewStore)
;usage: sets the-store to a state like the original, but with position ref containing val.
(define (prim-setref! store refval val)
...
(define (value-of-program pgm)
  ...
  (cases program pgm
    (a-program (exp1)
               (cases answer (value-of exp1 (init-nameless-env) the-store)
                 (an-answer (val store) val)))))

;value-of : Exp × NamelessEnv × Store → Answer
;we don't check the arity match for now.
(define value-of
  (lambda (exp env store)
    (cases expression exp
      (const-exp (num) (an-answer (num-val num) store))
      (nameless-var-exp (addr) (an-answer (apply-nameless-env env addr) store))
      (if-exp (exp1 exp2 exp3)
      ...
      (nameless-let-exp (exps body)
                        (let ((res (value-of-explist exps env store)))
                          (value-of body (extend-nameless-env+ (car res) env) (cdr res))))
      (nameless-let*-exp (exps body)
                         (let ((res (extend-nameless-env* exps env store)))
                           (value-of body (car res) (cdr res))))
      (nameless-letrec-exp (p-bodys letrec-body)
                           (value-of letrec-body (extend-nameless-env-rec p-bodys env) store))
      (null-exp () (an-answer (null-val) store))
      (cond-exp (preds actions)
                (value-of-cond preds actions env store))
      (nameless-unpack-exp (exp1 body)
                           (cases answer (value-of exp1 env store)
                             (an-answer (lst new-store)
                                        (value-of body (extend-nameless-env+ (expval->list lst) env) new-store))))
      (nameless-proc-exp (body) (an-answer (proc-val (compound body env #f)) store))
      (nameless-traceproc-exp (body) (an-answer (proc-val (compound body env #t)) store))
      (begin-exp (exp1 exps) (let loop ((exp1 exp1)
                               ...
                               (cases answer (value-of exp1 env store)
                                 (an-answer (val new-store)
                                            (if (null? exps)
                                                (an-answer val new-store)
                                                (loop (car exps) (cdr exps) new-store))))))
      (call-exp (rator rands)
                (cases answer (value-of rator env store)
                  (an-answer (procval store2)
                             (let ((res (value-of-explist rands env store2)))
                               (apply-procedure (expval->proc procval)
                                                (car res) (cdr res))))))
...
; ListOf(Exp) × Env × Store → PairOf(ListOf(ExpVal), Store)
(define (value-of-explist exps env store)
  ; keep the order of the ExpVals as the original exps
  ; so we need to reverse the resulting list of expvals
  (let ((res (fold/l (lambda (res exp)
                       (let ((vals (car res))
                             (sto (cdr res)))
                         (cases answer (value-of exp env sto)
                           (an-answer (val new-store)
                                      (cons (cons val vals) new-store)))))
                     (cons '() store) exps)))
    (cons (reverse (car res)) (cdr res))))

(define (value-of-cond preds actions env store)
  (define (iter preds actions store)
    (cond
      ((null? preds) (eopl:error 'value-of-cond "none of the cond predicates succeed"))
      (else (cases answer (value-of (car preds) env store)
              (an-answer (val new-store)
                         (if (equal? true val)
                             (value-of (car actions) env new-store)
                             (iter (cdr preds) (cdr actions) new-store)))))))
  (iter preds actions store))
...
(define (apply-procedure proc1 args store)
  (cases proc proc1
    (compound (body env trace)
              ...
              (let ((res (value-of body (extend-nameless-env+ args env) store)))
                (if trace (display "exit procedure\n") #t)
                res))
    (primitive (name op) (an-answer (apply op args) store))
    (store-prim (name op) (apply op store args))))
...
(define store-primitives
  (list (cons 'newref prim-newref)
