#lang eopl
; Exercise 3.18 [**] Add an expression: `Expression ::= unpack {Identifier}∗ = Expression in Expression`
...
; We should convert the value of `lst` into a scheme list so as to adapt to `extend-env+`
; but only convert one layer which means we don't recursively on the car.
...
;           in -(x,y)")
;#(struct:num-val 4)> (run "let u = 7
;in unpack x y = cons(u,cons(3,cons(4,emptylist)))
;in -(x,y)")
;unpack: the length of the list doesn't match the number of variables
;> (run "let u = 7
;in unpack x y = cons(u,emptylist)
;in -(x,y)")
;unpack: the length of the list doesn't match the number of variables