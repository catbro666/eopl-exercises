#lang eopl
; Continuation-Passing Interpreter (based on 3.35 version)
...
; To keep things clear and simple, we begin with the lauguage LETREC (Exercise 3.35 version).
; We add an additional continuation parameter to the `value-of` which makes the control context
; explicit. We can then use it to add new control behavior to our language.
