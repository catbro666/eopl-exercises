#lang eopl
; Exception (based on Exercise 5.35)

;Exercise 5.39 [**] So far, an exception handler can propagate the exception by reraising it,
;or it can return a value that becomes the value of the try expression. One might instead
;design the language to allow the computation to resume from the point at which the exception was raised.
;Modify the interpreter of this section to accomplish this by running the body of the handler
;with the continuation from the point at which the raise was invoked.

; save the continuation where the exception was raised and then run the body of handler with this saved continuation.
...
(define (apply-handler val cont saved-try-cont)
  (cases continuation saved-try-cont
    (try-cont (var handler-exp saved-env saved-cont)
              (value-of/k handler-exp (extend-env var val saved-env) cont)) ;*** use the cont passed in
