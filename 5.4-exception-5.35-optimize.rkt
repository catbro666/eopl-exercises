...
;Exercise 5.35 [**] This implementation is inefficient, because when an exception is raised,
;`apply-handler` must search linearly through the continuation to find a handler. Avoid this search
;by making the try-cont continuation available directly in each continuation.

; add a new field in each continuation (except `end-cont`, `try-cont` and `raise-cont`) which is the nearest `try-cont` continuation.
...
(define value-of/k
  (lambda (exp env cont)
    (cases expression exp
      ...
      (raise-exp (exp1)
                 (value-of/k exp1 env (raise-cont (get-saved-try-cont cont))))
      (call-exp (rator rands)
...
(define (get-saved-try-cont cont)
  (cases continuation cont
    ...
    (raise-cont (saved-try-cont) saved-try-cont)))
