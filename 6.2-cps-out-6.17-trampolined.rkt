...
(define rexp 'uninitialized)
(define renv 'uninitialized)
(define rproc 'uninitialized)
(define rargs 'uninitialized)
...
(define (value-of-program pgm)
  (cases program pgm
    (a-program (tfexp1)
               (set! rexp tfexp1)
               (set! renv (init-env))
               (trampoline! ((end-cont) (value-of/k))))))
      ...
      (cps-let-exp (vars rhss body)
                   (let* ((saved-env renv)
                          (vals (map (lambda (rhs)
                                       (set! rexp rhs)
                                       (set! renv saved-env)
                                       (value-of-simple-exp)) rhss)))
                     (set! rexp body)
                     (set! renv (extend-env+ vars vals saved-env))
                     (value-of/k)))
      (cps-letrec-exp (p-names b-varss p-bodies letrec-body)
                      (set! rexp letrec-body)
                      (set! renv (extend-env-rec** p-names b-varss p-bodies renv))
                      (value-of/k))
      (cps-if-exp (simple1 body1 body2)
                  (let ((saved-env renv))
                    (set! rexp simple1)
                    ...
                    (let ((saved-env renv))
                      (let ((rator-proc (begin
                            ...
                            (rand-vals (map (lambda (simple)
                                              ...
                                              (value-of-simple-exp))
                                            rands)))
                        (set! rproc rator-proc)
                        (set! rargs rand-vals)
                        (apply-procedure/k)))))))

;regs: rexp renv
(define (value-of-simple-exp)
  (cases simpleexp rexp
    (cps-const-exp (num) (num-val num))
    (cps-var-exp (var) (apply-env renv var))
    (cps-diff-exp (simp1 simp2) (prim-arith- (begin (set! rexp simp1)
                                                    (value-of-simple-exp))
                                             (begin (set! rexp simp2)
                                                    (value-of-simple-exp))))
    (cps-zero?-exp(simp1) (prim-zero? (begin (set! rexp simp1) (value-of-simple-exp))))
    (cps-proc-exp (vars body) (proc-val (compound vars body renv)))))
  ...
  (lambda ()
    (cases proc rproc
      (compound (vars body saved-env)
                (set! rexp body)
                (set! renv (extend-env+ vars rargs saved-env))
                (value-of/k))
      (primitive (name op) (apply op rargs)))))
