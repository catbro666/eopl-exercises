...
; Exercise 6.24 [**]  Add lists to the language (exercise 3.10).
; Remember that the arguments to a list are not in tail position.

; we add the general support of primitives
; +|-|*|/|equal?|greater?|less?|minus|cons|car|cdr|null?|list|print
; this will cause two changes:
; the primitive call now becomes a tfexp
; need to modify apply-procedure correspondingly
