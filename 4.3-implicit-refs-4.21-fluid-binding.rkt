#lang eopl
; Implicit References (Based on 4.20 version)

;Exercise 4.21 [**] Add to the language a facility for dynamic assignment (also called fluid binding)
; to accomplish this.

; The effect of the `setdynamic` expression is to assign temporarily the value of `exp1` to `var`, evaluate `body`,
; reassign `var` to its original value, and return the value of body. The variable var must already be bound.
...
;1
;(run "let x = 0 in set x = 1")
; error
(run "letm x = 11
      in let p = proc (y) (- y x)
         in (cons setdynamic x = 17 during (p 22) (p 13))")
;(5 . 2)