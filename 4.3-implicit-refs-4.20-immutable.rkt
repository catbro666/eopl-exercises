#lang eopl
; Implicit References (Based on 4.19 version)

;Exercise 4.20 [**] Modify the language of this section so that let introduces immutable variables, as
;                   before, but mutable variables are introduced by a `letm(utable)` expression

; add an additional parameter `mutable` to the `extend-nameless-env` series functions
    ...
    (cases expression exp
      ...
      (nameless-assign-exp (addr exp1)
                           (let ((denval (apply-nameless-env env addr)))
                             (if (reference? denval)   ;****
                                 (begin (setref! denval (value-of exp1 env))
                                        (void-val))
                                 (report-set-immutable-varaible exp1))))
      (call-exp (rator rands)
