#lang eopl
; Exception (based on 5.2-trampolined-interpreter)

;Exercise 5.36 [*] An alternative design that also avoids the linear search in applyhandler is to use two continuations,
;a normal continuation and an exception continuation. 
...
(define (value-of-program pgm)
  ...
  (cases program pgm
    (a-program (exp1)
               (trampoline (value-of/k exp1 (init-env) (end-cont) (end-cont))))))
...
(define value-of/k
  (lambda (exp env cont exc-cont)
    (cases expression exp
      (const-exp (num) (apply-cont cont exc-cont (num-val num)))
      (var-exp (var) (apply-cont cont exc-cont (deref (apply-env env var))))
      (if-exp (exp1 exp2 exp3)
              (value-of/k exp1 env
                          (if-test-cont exp2 exp3 env cont) exc-cont))
      (let-exp (vars exps body)
               (value-of-explist/k exps '() env
                                   (explist-final-cont vars body env cont) exc-cont))
      (let*-exp (vars exps body)
                (value-of-explist*/k vars exps env
                                     (explist*-final-cont body cont) exc-cont))
      (letrec-exp (p-names b-varss p-bodys letrec-body)
                  (value-of-explist-rec/k p-names b-varss p-bodys env
                                          (explist-rec-final-cont letrec-body cont) exc-cont))
      (null-exp () (apply-cont cont exc-cont (null-val)))
      (cond-exp (preds actions)
                (value-of-cond/k preds actions env cont exc-cont))
      (unpack-exp (vars exp1 body)
                  (value-of/k exp1 env
                              (unpack-exp-cont vars body env cont) exc-cont))
      (proc-exp (vars body) (apply-cont cont exc-cont (proc-val (compound vars body env #f))))
      (traceproc-exp (vars body) (apply-cont cont exc-cont (proc-val (compound vars body env #t))))
      (begin-exp (exp1 exps)
                 (value-of-begin/k exp1 exps env cont exc-cont))
      (try-exp (exp1 var handler-exp)
               (let ((new-cont (try-cont var handler-exp env cont exc-cont)))
                 (value-of/k exp1 env new-cont new-cont)))
      (raise-exp (exp1)
                 (value-of/k exp1 env (raise-cont) exc-cont))
      (call-exp (rator rands)
...
(define (apply-cont cont exc-cont val)
  (cases continuation cont
    (end-cont () (eopl:printf "End of computation.~%") val)
    (if-test-cont (exp2 exp3 saved-env saved-cont)
                  (if (expval->bool val)
                      (value-of/k exp2 saved-env saved-cont exc-cont)
                      (value-of/k exp3 saved-env saved-cont exc-cont)))
    (explist-final-cont (vars body saved-env saved-cont)
                  (value-of/k body (extend-env+ vars val saved-env) saved-cont exc-cont))
    (explist-cont (exps vals saved-env saved-cont)
                  (value-of-explist/k exps (cons val vals) saved-env saved-cont exc-cont))
    (explist*-final-cont (body saved-cont)
                  (value-of/k body val saved-cont exc-cont))
    (explist*-cont (vars exps saved-env saved-cont)
                  (value-of-explist*/k (cdr vars) exps (extend-env (car vars) val saved-env) saved-cont exc-cont))
    (explist-rec-final-cont (body saved-cont)
                  (value-of/k body val saved-cont exc-cont))
    (cond-exp-cont (preds actions saved-env saved-cont)
                   (if (equal? true val)
                       (value-of/k (car actions) saved-env saved-cont exc-cont)
                       (value-of-cond/k preds (cdr actions) saved-env saved-cont exc-cont)))
    (unpack-exp-cont (vars body saved-env saved-cont)
                     (let ((vals (expval->list val)))
                       (if (equal-length? vars vals)
                           (value-of/k body (extend-env+ vars vals saved-env) saved-cont exc-cont)
                           (eopl:error 'unpack "the length of the list doesn't match the number of variables"))))
    (begin-exp-cont (exps saved-env saved-cont)
                    (value-of-begin/k (car exps) (cdr exps) saved-env saved-cont exc-cont))
    (rator-cont (rands saved-env saved-cont)
                (let ((rator-proc (expval->proc val)))
                  (value-of-explist/k rands '() saved-env
                                      (rands-cont rator-proc saved-cont)
                                      exc-cont)))
    (rands-cont (proc1 saved-cont)
                (apply-procedure/k proc1 val saved-cont exc-cont))
    (trace-cont (trace saved-cont)
                (if trace (display "exit procedure\n") #t)
                (apply-cont saved-cont exc-cont val))
    (try-cont (var handler-exp saved-env saved-cont saved-exc-cont)
              (apply-cont saved-cont saved-exc-cont val)) ; use `saved-exc-cont`
    (raise-cont ()
                (apply-handler val exc-cont))))

;apply-handler : ExpVal × Cont → FinalAnswer
(define (apply-handler val cont)
  (cases continuation cont
    (try-cont (var handler-exp saved-env saved-cont saved-exc-cont)
              (value-of/k handler-exp (extend-env var val saved-env) saved-cont saved-exc-cont)) ;***
...
(define (value-of-explist/k exps vals env cont exc-cont)
  (if (null? exps)
      (apply-cont cont exc-cont (reverse vals))
      (value-of/k (car exps) env (explist-cont (cdr exps) vals env cont) exc-cont)))

(define (value-of-explist*/k vars exps env cont exc-cont)
  (if (null? exps)
      (apply-cont cont exc-cont env)
      (value-of/k (car exps) env (explist*-cont vars (cdr exps) env cont) exc-cont)))

(define (value-of-explist-rec/k p-names b-varss bodys env cont exc-cont)
  (let* ((vecs (map (lambda (n) (make-vector 1)) p-names))
    ...
    (for-each (lambda (vec b-vars body)
              ...
              vecs b-varss bodys)
    (apply-cont cont exc-cont new-env)))

(define (value-of-cond/k preds actions env cont exc-cont)
  (if (null? preds)
      (eopl:error 'value-of-cond/k "none of the cond predicates succeed")
      (value-of/k (car preds) env
                  (cond-exp-cont (cdr preds) actions env cont) exc-cont)))

(define (value-of-begin/k exp1 exps env cont exc-cont)
  (if (null? exps)
      (value-of/k exp1 env cont exc-cont)
      (value-of/k exp1 env (begin-exp-cont exps env cont) exc-cont)))
...
(define (apply-procedure/k proc1 args cont exc-cont)
  (lambda ()
    (cases proc proc1
      (compound (vars body env trace)
                ...
                (value-of/k body (extend-env+ vars args env)
                            (trace-cont trace cont) exc-cont))
      (primitive (name op) (apply-cont cont exc-cont (apply op args))))))
