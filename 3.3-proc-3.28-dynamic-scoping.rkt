#lang eopl
; Exercise 3.28 [**]  Dynamic binding (or dynamic scoping) is an alternative design for procedures, in which
; the procedure body is evaluated in an environment obtained by extending the environment at the point of call. 

; the procedure datatype does not need to contain env any more.
...
(define-datatype proc proc?
  (compound (vars (list-of identifier?)) (body expression?) (trace boolean?))
...
(define value-of
  (lambda (exp env)
    (cases expression exp
      ...
      (proc-exp (vars body) (proc-val (compound vars body #f)))
      (traceproc-exp (vars body) (proc-val (compound vars body #t)))
      (call-exp (rator rands)
                (let ((a-proc (expval->proc (value-of rator env)))
                      (args (value-of-explist rands env)))
                  (apply-procedure a-proc args env)))
      )))
...
(define (apply-procedure proc1 args env)
  (cases proc proc1
    (compound (vars body trace)
